...
def evaluate(gold_fpath, pred_fpath, thresholds=None):
    ...
    ranked_lines = [t[0]
                    for t in sorted(line_score, key=lambda x: x[1], reverse=True)]
    if thresholds is None or len(thresholds) == 0:
        thresholds = MAIN_THRESHOLDS + [len(ranked_lines)]

    # Calculate Metrics
    precisions = _compute_precisions(
        gold_labels, ranked_lines, len(ranked_lines))
...
def get_threshold_line_format(thresholds, last_entry_name):
    threshold_line_format = '{:<30}' + \
        "".join(['@{:<9}'.format(ind) for ind in thresholds])
    if last_entry_name:
        threshold_line_format = threshold_line_format + \
            '{:<9}'.format(last_entry_name)
...
def print_thresholded_metric(title, thresholds, data, last_entry_name=None, last_entry_value=None):
    line_separator = '=' * 120
    threshold_line_format = get_threshold_line_format(
        thresholds, last_entry_name)
    items = data
    ...
    logging.info('{:<30}'.format("") +
                 "".join(['{0:<10.4f}'.format(item) for item in items]))
...
def print_metrics_info(line_separator):
    ...
    logging.info(
        '!!! THE OFFICIAL METRIC USED FOR THE COMPETITION RANKING IS MEAN AVERAGE PRECISION (MAP) !!!')
    logging.info(
        'R-Precision is Precision at R, where R is the number of relevant line_numbers for the evaluated set.')
    ...
    logging.info(
        'Precision@N is precision estimated for the first N line_numbers in the provided ranked list.')
    logging.info(
        'The MEAN versions of each metric are provided to average over multiple debates (each with separate prediction file).')
    logging.info(line_separator)
    logging.info(line_separator)
